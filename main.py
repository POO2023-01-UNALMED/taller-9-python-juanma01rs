...
def boton(evento):
    global inp
    texto_boton = str(evento.widget["text"])
    pantalla.insert("end", texto_boton)
    inp += texto_boton

def operador(evento):
    global inp
    texto_operador = str(evento.widget["text"])
    pantalla.insert("end", texto_operador)
    inp += texto_operador
...
boton_9.grid(row=3, column=2, padx=0, pady=1)
